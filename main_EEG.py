...
for cndt in conditions:
    for sbj in subjects:
        ...
        X = np.array(
            [data[list(range(i + int(start * f_sample), i + int(end * f_sample))), :] for i in x_event_time_indices])
...
for model_name, train_callback in model_name_train_callback_dict.items():
    history, clsf_rpt = train_callback(x_train, x_test, y_train, y_test, encoder, note=model_name + ' ' + scenario,
                                       patience=25, )
